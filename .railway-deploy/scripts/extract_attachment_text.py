                ...
                try:
                    ...
                    for row in root.iter("row"):
                        ...
                        for cell in row.iter("c"):
                            ...
                            # Position-aware placement: parse the cell reference (r="B5")
                            # so empty/gap cells keep column alignment.
                            ref = cell.attrib.get("r", "")
                            col = _column_ref_to_index(ref) if ref else next_col
                            if col < 0:
                                col = next_col
                            cells[col] = value
                            next_col = col + 1
                        if not cells:
                            continue
                        # Drop trailing empties only; keep interior/leading gaps.
                        end = max(cells) + 1
                        while end > 0 and not cells.get(end - 1):
                            end -= 1
                        if end == 0:
                            continue
                        values = [cells.get(i, "") for i in range(end)]
