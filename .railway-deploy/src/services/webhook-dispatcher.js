...
export class LegacyWebhookDispatcher {
  ...
  async enqueueProjectMessages(projectId, messages = []) {
    ...
    const deliveries = messages.flatMap((message) => getWebhookClients(this.integrationClients, project.id, message)
      .map((client) => createWebhookDelivery(project, message, client, this.baseUrl, this.attachmentToken)));
...
export function createWebhookDelivery(project, message, client, baseUrl = "", attachmentToken = "") {
  ...
  const payload = {
    ...
    data: normalizeIntegrationMessage(project, message, { consumerId: client.id, baseUrl, attachmentToken })
  };
