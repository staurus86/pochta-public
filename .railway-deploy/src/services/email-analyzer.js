...
function sanitizePersonName(raw) {
  ...
  if (ORG_LEGAL_FORM_RE.test(trimmed)) return null;
  if (JOB_TITLE_STOPLIST.test(trimmed)) return null;
  // Multiline block: if there is any newline, take the first non-junk line only.
  if (trimmed.includes("\n")) {
    const firstLine = trimmed.split(/\n/).map((s) => s.trim()).find((s) => s.length > 0);
    if (!firstLine) return null;
    // re-validate first line
    if (ORG_LEGAL_FORM_RE.test(firstLine) || JOB_TITLE_STOPLIST.test(firstLine)) return null;
    return firstLine.length <= 80 ? firstLine : null;
  }
  // Hard length cap: real names are <= 60 chars. Anything longer is a bled signature fragment.
