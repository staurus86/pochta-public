...
1) Служебное письмо (автоответ, «сообщение не доставлено», OOO) → "Не определено".
2) Отправитель предлагает СВОИ товары/услуги → "Поставщик услуг".
3) Рассылка, реклама, webinar, newsletter → "СПАМ".
4) Конкретный запрос цены / заказа / уточнения по позиции → "Клиент".
5) Пересылка (Fwd/Fw): классифицируй по ОРИГИНАЛУ внутри цитаты, не по заголовку пересылающего.
