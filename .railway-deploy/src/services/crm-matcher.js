...
export function matchCompanyInCrm(project, analysis) {
  ...
  const senderKpp = analysis.sender.kpp;
  const exactMatch =
    // Improvement 5: prefer INN+KPP match (branch-aware) then fall back to INN-only
    (senderInn && senderKpp && companyCandidates.find((company) => company.inn === senderInn && company.kpp === senderKpp)) ||
