...
function findSemanticNomenclatureMatches(query, bodyText = "") {
  ...
  for (const tokenQuery of tokenQueries) {
    for (const item of detectionKb.searchNomenclature(tokenQuery, { limit: 3 })) {
      ...
      // article code appears verbatim in the full email body. Without this, SQLite FTS
      // over catalog descriptions returned 100+ false brands per inbox — any industrial
      // email with generic tokens like "power", "control", "electrical" matched multi-token
      // catalog descriptions (Elec-Con, PACE Worldwide, Tele Radio, Micro*× 5, IREM, etc.)
      // despite those brands never being mentioned. Semantic match is a fallback for emails
      // with zero detected brands — require at least one grounded token.
