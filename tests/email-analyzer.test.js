...
runTest("stripBrandCapabilityList: Siderus signature brand catalog не попадает в detectedBrands", () => {
  const analysis = analyzeEmail(project, {
    fromName: "Денис Акимов",
    fromEmail: "277@siderus.ru",
    subject: "RE: Запрос на ABB S201",
    attachments: "",
    body: [
      "Добрый день.",
      "Подтверждаем наличие ABB S201-C16 — 5 шт.",
      "",
      "С уважением,",
      "Денис Акимов",
      "Менеджер отдела продаж",
      "ООО \"КОЛОВРАТ\"",
      "",
      "Бренды, по которым мы наиболее активно работаем.",
      "",
      "Brevetti Stendalto, Effegi Brevetti, Garlock, GEFA Processtechnik, Groschopp, HAUG, Igema, IRON Pump, KNOLL, Keyence, LAIPPLE KEB, Lamb Electric, MISUMI, Moog, Penny & Giles, REMBE, SERAD, Tandler, TAKEX, Tamagawa, VENTAIX, Zimmer Automation, Zollern, Brooks Instrument, Eckerle Hydraulic, SCHLESINGER, Kipp & Zonen, LEAX, Pioneer Magnetics, Zimmerlin, ASTRO Motoren, AW Lake, EBBCO, Eurotherm Drives, NIMAK, Tempco, RECO, Gualapack, Agrichema, Serfilco, Nova Rotors, Forkardt, Schimpf, Vuototecnica, Murrelektronik, IFM, SICK, Datalogic, Euchner, Lenze, Stromag, Beckhoff, Status Pro, GSR, Elap, Kobold, KEB, Zorzini, JKF"
    ].join("\n")
  });
  const brands = analysis.lead.detectedBrands || [];
  // Должен остаться только ABB (из реального запроса), а не 50+ брендов из каталога
  assert.ok(brands.length < 5, `Ожидалось < 5 брендов, получено ${brands.length}: ${brands.join(", ")}`);
  const noCatalogBrands = !brands.some((b) => /brevetti|garlock|misumi|penny|tamagawa|beckhoff|zimmer|forkardt|ebbco/i.test(b));
  assert.ok(noCatalogBrands, `Бренды из каталога подписи не должны попадать: ${brands.join(", ")}`);
});

runTest("stripBrandCapabilityList: английский вариант Brands we work with", () => {
  const analysis = analyzeEmail(project, {
    fromName: "John Doe",
    fromEmail: "john@example.com",
    subject: "Inquiry for ABB",
    attachments: "",
    body: [
      "Hi, we need ABB S201-C16 x10.",
      "",
      "Regards, John",
      "",
      "Brands we work with: Siemens, Allen-Bradley, Omron, Mitsubishi, Phoenix Contact, Weidmüller, Beckhoff, Beckhoff, Balluff, Pepperl+Fuchs, Endress+Hauser"
    ].join("\n")
  });
  const brands = analysis.lead.detectedBrands || [];
...
runTest("signature-brand-cluster: comma-chain в подписи БЕЗ маркера удаляется, легитимный бренд запроса сохраняется", () => {
  // Electrovent/АИСС-стиль: клиент запрашивает 1 товар, в подписи comma-separated
  // список 20+ брендов которые они поставляют. Post-hoc position cluster должен
  // удалить цепочку, сохранив Grundfos (упомянут до подписи в теле запроса).
  const analysis = analyzeEmail(project, {
    fromName: "Иванов П",
    fromEmail: "303@electrovent.ru",
    subject: "ЗАЯВКА",
    attachments: "",
    body: [
      "Добрый день. Сможете прислать предложение:",
      "0001 Насос Grundfos SQ 3-65 — 2 шт.",
      "",
      "С уважением, Иванов Павел.",
      "https://electrovent.ru",
      "ПРИГЛАШАЕМ ВАС В ЭДО: 2BE13af72e7d2da4894ac392c4a6b43b936",
      "ABB, Siemens, Schneider Electric, Phoenix Contact, Belimo, Danfoss, Leuze, Lenze, Sick, Turck, Ifm, Beckhoff, Balluff, Pepperl+Fuchs, Vega, Emerson, Autonics, Eaton, Omron, Fanuc, Heidenhain, Mean Well, Parker, Rexroth, Weidmuller, Wilo"
    ].join("\n")
  });
  const brands = (analysis.lead.detectedBrands || []).map((b) => b.toLowerCase());
  // Grundfos — из запроса, должен остаться
  assert.ok(brands.some((b) => /grundfos/.test(b)), `Grundfos (легитимный) должен детектиться, получено: ${brands.join(", ")}`);
  // Siemens/ABB/Schneider — из signature chain, должны быть удалены
  assert.ok(!brands.some((b) => b === "siemens"), `Siemens из подписи не должен попадать, получено: ${brands.join(", ")}`);
  assert.ok(!brands.some((b) => b === "abb"), `ABB из подписи не должен попадать, получено: ${brands.join(", ")}`);
  // Общее количество должно быть сильно меньше 20+ (сильная редукция)
  assert.ok(brands.length < 10, `должно остаться <10 брендов, получено ${brands.length}: ${brands.join(", ")}`);
});

runTest("semantic-token: английский маркетинговый спам не получает ложных брендов через FTS", () => {
  // Регрессия (pochta-inbox-2026-04-18 N=88 CESP): английское рекламное письмо из .cn
  // про "Explosion-Proof LED Lights" получало 27 брендов (Elec-Con, PACE Worldwide,
  // Tele Radio, MICRO MOTION/EPSILON/Matic/Motors/Measurements, Lang & Schmidt,
  // POWER-GENEX, POWER-HYDRAULIK, IREM и т.д.), 23 из которых в теле отсутствовали.
  // Причина: semantic FTS возвращал rows с общими токенами (power, control, electrical,
  // lighting), overlap≥3 проходил легко. Гейт «бренд или артикул в теле» блокирует.
  const analysis = analyzeEmail(project, {
    fromName: "CESP",
    fromEmail: "cespledex@gtdwcry.cn",
    subject: "Reduce Safety Risks with Certified Explosion-Proof LED Lights",
    attachments: "",
    body: [
      "Dear Friend,",
      "",
      ...
      "",
      "We are a professional manufacturer of LED Explosion-Proof Lighting Solutions,",
      "engineered specifically for hazardous and flammable working environments where",
      "safety, reliability, and efficiency are top priorities.",
      "All our products are fully certified with UL, ATEX, and IECEx, complying with",
      "global safety standards for explosive gas and dust locations.",
      "",
      "With power options from 20W to 380W, we meet the needs of various industrial projects."
    ].join("\n")
  });
  const brands = (analysis.lead.detectedBrands || []).map((b) => b.toLowerCase());
  const fpBrands = ["elec-con", "pace worldwide", "tele radio", "micro motion", "micro-epsilon",
                    "micro matic", "micro motors", "micro-measurements", "lang & schmidt",
                    "power-genex", "power-hydraulik", "irem", "link belt", "robot pumps"];
  for (const fp of fpBrands) {
    assert.ok(
      !brands.some((b) => b.includes(fp)),
      `FP бренд "${fp}" не должен детектиться через semantic (его нет в теле), получено: ${brands.join(", ")}`
    );
  }
});

runTest("article-noise: RTF control words не попадают в артикулы", () => {
  // Регрессия (pochta-inbox-2026-04-18 N=103): Word/Outlook письмо с RTF preamble
  // в теле генерировало 87 артикулов где 61 — RTF control words
  // (RTF1, FCHARSET204, PAPERW11906, NOFWORDS62, VIEWKIND1, RSID146116, PNSECLVL1,
  // SBASEDON10, TRFTSWIDTHB3, DEFTAB708, LSDSTIMAX156, EDMINS1, NOFCHARS358, VERN24703).
  const analysis = analyzeEmail(project, {
    fromName: "Николай",
    fromEmail: "recepson@mail.ru",
    subject: "Re: FW: Заполнена форма Товар под заказ",
    attachments: "",
    body: [
      "Артикул: AT 051 DA F04 N 11 DS — 2 шт.",
      "",
      "{\\rtf1\\ansi\\ansicpg1251\\cocoartf2758",
      "\\cocoatextscalingfactor1\\cocoaplatformversion1300",
      "{\\fonttbl\\f0\\fnil\\fcharset204 ArialMT;\\f1\\fnil\\fcharset0 Times;",
      "\\f410\\froman\\fcharset238 TimesNewRomanPSMT;\\f408\\fmodern\\fcharset161 Courier;}",
      "{\\colortbl;\\red255\\green255\\blue255;\\red0\\green0\\blue0;}",
      "\\paperw11906\\paperh16838\\margl1134\\margr850\\margt1134\\margb1134",
      "\\deftab708\\viewkind1\\viewscale100\\pgnstart1\\pnsectlevel1\\pnsectlevel2",
      "\\rsid146116\\rsid2176431\\nofpages1\\nofwords62\\nofchars358 vern24703"
    ].join("\n")
  });
  const articles = (analysis.lead.articles || []).map((a) => a.toUpperCase());
  const rtfTokens = ["RTF1", "FCHARSET204", "FCHARSET0", "FCHARSET238", "FCHARSET161",
                     "PAPERW11906", "DEFTAB708", "VIEWKIND1", "RSID146116", "RSID2176431",
                     "NOFPAGES1", "NOFWORDS62", "NOFCHARS358", "VERN24703"];
  for (const tok of rtfTokens) {
    assert.ok(
      !articles.includes(tok),
      `RTF control word "${tok}" не должен попадать в артикулы, получено: ${articles.join(", ")}`
    );
  }
});

runTest("article-noise: POS.N (номер позиции) не попадает в артикулы", () => {
  // Регрессия (N=1264 JSW): "заявка на запчасти для оборудования JSW" с позиционной нумерацией
  // даёт 112 артикулов где 12+ это POS.12, POS.14, pos.22 и т.д.
  const analysis = analyzeEmail(project, {
    fromName: "Илья Котиков",
    fromEmail: "kotikovin.ngs@gmail.com",
    subject: "заявка на запчасти для оборудования JSW",
    attachments: "",
    body: [
      "Добрый день, нужны следующие позиции:",
      "POS.12 — K4A4J0-A71-029 Gasket 5 шт.",
      "POS.14 — PA4700043900-1 Piston ring 10 шт.",
      "POS.15 — PA4700043900-2 Piston ring 10 шт.",
      "pos.22 — CMP-274 2 шт.",
      "POS.54 — JIS-T3-P50A Back up 3 шт."
    ].join("\n")
  });
  const articles = (analysis.lead.articles || []).map((a) => a.toUpperCase());
  assert.ok(!articles.some((a) => /^POS\.\d+$/i.test(a)),
    `POS.N не должен быть артикулом, получено: ${articles.join(", ")}`);
  // Control: реальные артикулы должны сохраниться
  assert.ok(articles.some((a) => /K4A4J0|PA4700043900|CMP-274/i.test(a)),
    `Реальные артикулы должны остаться, получено: ${articles.join(", ")}`);
});

runTest("article-noise: N.N.N list numbering (1.3.1, 1.3.2, 1.3.3) не попадает в артикулы при ≥3 подряд", () => {
  // Регрессия (N=827): outline numbering '1.3.1', '1.3.2', '1.3.3' ловилось как артикулы.
  // Контекстный фильтр: ≥3 токена N.N.N с сегментами ≤99 → это нумерация пунктов.
  const analysis = analyzeEmail(project, {
    fromName: "Заказчик",
    fromEmail: "buyer@example.ru",
    ...
    body: [
      "Техническое задание:",
      "1.3.1 Монтаж оборудования",
      "1.3.2 Пуско-наладочные работы",
      "1.3.3 Обучение персонала",
      "1.3.4 Гарантийное обслуживание",
      "Артикул: ABB 1SDA012345R1"
    ].join("\n")
  });
  const articles = (analysis.lead.articles || []).map((a) => String(a));
  assert.ok(!articles.some((a) => /^\d+\.\d+\.\d+$/.test(a) && a !== "504.186.202"),
    `N.N.N нумерация не должна быть артикулом, получено: ${articles.join(", ")}`);
});

runTest("article-noise: одиночный Festo-style N.N.N (504.186.202) сохраняется как артикул", () => {
  // Контроль: реальные Festo-артикулы с 3-значными сегментами не должны удаляться.
  // Фильтр срабатывает только при ≥3 токенах N.N.N с сегментами ≤99.
  const analysis = analyzeEmail(project, {
    fromName: "Заказчик",
    fromEmail: "buyer@example.ru",
    subject: "Запрос Festo",
    attachments: "",
    body: [
      "Добрый день,",
      "Нужен артикул Festo 504.186.202 в количестве 2 шт.",
      "С уважением"
    ].join("\n")
  });
  const articles = (analysis.lead.articles || []).map((a) => String(a));
  assert.ok(articles.some((a) => /504\.186\.202/.test(a)),
    `Festo 504.186.202 должен сохраниться, получено: ${articles.join(", ")}`);
});

runTest("internal-sender: own-domain mailbox (106@siderus.ru) попадает на review, не auto-sync", () => {
  // Регрессия: внутренние письма от 106@siderus.ru, 138@siderus.ru и т.д. (нумерованные mailbox'ы
  // сотрудников) классифицируются как Клиент и уходят в CRM. Должны требовать review.
  const analysis = analyzeEmail(project, {
    fromName: "Станислав Корычев",
    fromEmail: "106@siderus.ru",
    subject: "RE: запрос",
    attachments: "",
    body: [
      "Да, давай возьмем.",
  ...
  assert.equal(analysis.intakeFlow?.requiresReview, true,
    `own-domain sender должен requiresReview=true, получено: ${analysis.intakeFlow?.requiresReview}`);
  assert.equal(analysis.intakeFlow?.reviewReason, "internal_sender",
    `reviewReason должен быть internal_sender, получено: ${analysis.intakeFlow?.reviewReason}`);
  assert.ok((analysis.intakeFlow?.flags || []).includes("internal_sender"),
    `flags должны включать internal_sender, получено: ${JSON.stringify(analysis.intakeFlow?.flags)}`);
  assert.equal(analysis.intakeFlow?.createRequestInCrm, false,
    `createRequestInCrm должен быть false для internal, получено: ${analysis.intakeFlow?.createRequestInCrm}`);
});

runTest("internal-sender: robot@siderus.ru НЕ помечается internal (форма)", () => {
  // Контроль: robot@siderus.ru — website form, валидный клиентский запрос, не internal.
  const analysis = analyzeEmail(project, {
    fromName: "robot",
    fromEmail: "robot@siderus.ru",
    ...
    attachments: "",
    body: [
      'Заполнена форма "Товар под заказ" на сайте SIDERUS (8340)',
      "Имя посетителя: Иван Петров",
      "Телефон: +7 903 111-22-33",
      "Email: buyer@clientcorp.ru",
      "Название товара: ABB 1SDA012345R1",
      "Сообщение: Нужен в количестве 5 шт"
    ].join("\n")
  });
  assert.ok(!(analysis.intakeFlow?.flags || []).includes("internal_sender"),
    `robot@ forms не должны иметь internal_sender flag, получено: ${JSON.stringify(analysis.intakeFlow?.flags)}`);
});

runTest("article-noise: единицы измерения (1200V, 75A, 380W, 60HZ) не попадают в артикулы", () => {
  // Регрессия: параметры оборудования ловятся как артикулы (99 токенов в inbox).
  const analysis = analyzeEmail(project, {
    fromName: "Инженер",
    fromEmail: "eng@example.ru",
    subject: "Запрос двигателя",
    attachments: "",
    body: [
      "Добрый день,",
      "Нужен двигатель со следующими параметрами:",
      "Напряжение: 380V",
      "Ток: 75A",
      "Мощность: 1200W",
      "Частота: 60HZ",
  ...
  const noiseFound = articles.filter((a) => /^(?:380V|75A|1200W|60HZ|100-240V|4-20MA)$/i.test(a));
  assert.ok(noiseFound.length === 0,
    `Единицы не должны быть артикулами, получены: ${noiseFound.join(", ")} (all: ${articles.join(", ")})`);
});

runTest("article-noise: DN NN nominal diameter (DN 65, DN32) не попадает в артикулы", () => {
  const analysis = analyzeEmail(project, {
    fromName: "Инженер",
    fromEmail: "eng@example.ru",
    subject: "Запрос трубопроводной арматуры",
    attachments: "",
    body: [
      "Добрый день,",
      "Клапан DN 65 PN 16, Danfoss VFG-21",
      "Фланец DN32 ГОСТ 12820"
    ].join("\n")
  });
  const articles = (analysis.lead.articles || []).map((a) => String(a).toUpperCase());
  assert.ok(!articles.some((a) => /^DN\s*\d+/i.test(a)),
    `DN NN не должен быть артикулом, получено: ${articles.join(", ")}`);
});

runTest("signature-brand-cluster: малое число брендов не фильтруется (threshold=10)", () => {
  // Контроль: 3-4 бренда в одной строке — legitimate multi-brand запрос, не signature.
  const analysis = analyzeEmail(project, {
    fromName: "Клиент",
    fromEmail: "client@example.ru",
    subject: "Запрос на Danfoss, Siemens, Grundfos",
    attachments: "",
    body: [
      "Добрый день,",
      "",
      "Нужны насосы Danfoss VLT, Siemens 6SE, Grundfos SQ. Все с артикулами.",
      "",
      "С уважением"
    ].join("\n")
  });
  const brands = (analysis.lead.detectedBrands || []).map((b) => b.toLowerCase());
  assert.ok(brands.some((b) => /danfoss/.test(b)), `Danfoss должен остаться: ${brands.join(", ")}`);
